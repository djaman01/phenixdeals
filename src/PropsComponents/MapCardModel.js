...
export default function MapCardModel({ title, value, onChange, placeholder, error, filteredProducts, paginationStyle }) {
  ...
  //!!!!! Nouvelle variable sur laquelle on va mapper = currentItems / On utilise props pour ensuite mapper sur la function qu'on veut dans d'autres components 
  ...
  return (
    <>
      ...
      <div className='map-card-products'>
        {error ? (<p>Error: {error}</p>) : (
          ...
          <div className="grid-map-products" data-aos='zoom-in'>
            {/* il faut uiliser currentItems.map car on va filtrer dans les produits database et on veut voir la pagination: currentItems= filtredProducts.slice() */}
                ...
                <div className="item-map-products">
                  ...
                  <div className="div-thumbnail-map-products">
                    <img
                      ...
                      src={`https://phenixdeals.onrender.com/${item.imageUrl}`}//On store le path de l'image dans la database,, donc c'est ce qu'il faut chercher
                      alt={item.nom}
                    />
                  </div>
      ...
      <div className="div-pagination">
        <div className={`all-pagination ${paginationStyle}`}>
